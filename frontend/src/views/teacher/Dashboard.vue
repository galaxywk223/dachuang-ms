...
type TeacherProjectRow = ProjectInfo & {
  ...
  file_url?: string;
  file_label?: string;
  file_field?: "proposal_file" | "mid_term_report" | "final_report";
  file_name?: string;
  created_at?: string;
  ...
  try {
    if (activeTab.value === "pending") {
      ...
      const rows = (results || []).map((r) => {
        ...
        const fileName =
          r.review_type === "MID_TERM"
            ? r.project_info?.mid_term_report_name
            : r.review_type === "CLOSURE"
              ? r.project_info?.final_report_name
              : r.project_info?.proposal_file_name;
        const fileField =
          r.review_type === "MID_TERM"
            ? "mid_term_report"
            : r.review_type === "CLOSURE"
      ...
      if (!isExpertContext.value) {
        ...
        const changeRows: TeacherProjectRow[] = changeList.map((c) => ({
          ...
          file_label: "下载附件",
          file_name: (c.attachment_name as string) || "异动附件",
...
const downloadCurrentFile = async () => {
  const row = currentProject.value;
  if (!row) return;
  try {
    if (row.is_change_request && row.change_request_id) {
      const blob = (await downloadChangeRequestAttachment(
        row.change_request_id
      )) as Blob;
      saveBlob(blob, row.file_name || "异动附件");
      return;
    }
    if (!row.id || !row.file_field) return;
    const blob = (await downloadProjectFile(row.id, row.file_field)) as Blob;
    saveBlob(blob, row.file_name || row.file_label || "项目文件");
  } catch {
