  ...
  <div class="review-page">
    ...
    <div class="table-container">
      ...
      >
            ...
            <div class="expand-content">
              <el-descriptions title="项目详情" :column="2" border size="small">
                ...
                <el-descriptions-item label="指导老师" :span="2">
                  <span v-for="(advisor, index) in row.advisors_info" :key="advisor.id">
                    {{ advisor.name }} ({{ advisor.title }}){{ index < row.advisors_info.length - 1 ? '、' : '' }}
                  </span>
                </el-descriptions-item>
                <el-descriptions-item label="项目简介" :span="2">
                  <div class="text-content">{{ row.description || "暂无" }}</div>
                </el-descriptions-item>
                 <el-descriptions-item label="预期成果" :span="2">
                  <div class="text-content">{{ row.expected_results || "暂无" }}</div>
                </el-descriptions-item>
              </el-descriptions>

              <div class="review-actions" v-if="row.status === 'SUBMITTED' || row.status.includes('REVIEWING')">
                 <el-button type="success" :icon="Check" plain @click="handleApprove(row)">通过申请</el-button>
                 <el-button type="danger" :icon="Close" plain @click="handleReject(row)">驳回申请</el-button>
              </div>
            </div>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="created_at" label="申请日期" width="140" align="center">
             <template #default="{ row }">
...
import { Search, Check, Close, RefreshLeft, ArrowDown } from "@element-plus/icons-vue";
