        ...
        <el-table-column prop="proof_file_url" label="凭证" width="90" align="center">
          <template #default="scope">
            <el-link
              ...
              type="primary"
              :underline="false"
              :disabled="downloadingProofId === scope.row.id"
              @click="downloadProofFile(scope.row)"
            >
              查看
            </el-link>
...
import { downloadProjectExpenditureProof } from "@/api/projects";
...
const downloadingProofId = ref<number | null>(null);
...
const downloadProofFile = async (row: ExpenditureItem) => {
  if (!row.id) return;
  downloadingProofId.value = row.id;
  try {
    const blob = (await downloadProjectExpenditureProof(row.id)) as Blob;
    saveBlob(blob, row.proof_file_name || row.title || "经费凭证");
  } catch {
    ElMessage.error("下载经费凭证失败");
  } finally {
