...
export function useSystemDictionaries(
  ...
  const canClearCurrentType = computed(
    () => Boolean(currentType.value) && !currentType.value?.is_system
  );
  ...
  const clearItems = async () => {
    ...
    if (!type) return;
    if (!canClearCurrentType.value) {
      ElMessage.warning("系统内置参数不支持一键清空");
      return;
    }
    try {
  ...
  const deleteItem = async (item: DictionaryItem) => {
    if (!canDeleteItems.value) {
      ElMessage.warning("系统内置参数条目不支持直接删除，可改为停用");
      return;
    }
    try {
      await ElMessageBox.confirm(
