            ...
            <el-form-item label="证书底图">
              <div class="upload-row">
                ...
                <el-link
                  ...
                  type="primary"
                  @click="downloadResource('background_image')"
                >
                  下载当前底图
                </el-link>
              </div>
            ...
            <el-form-item label="电子印章">
              <div class="upload-row">
                ...
                <el-link
                  ...
                  type="primary"
                  @click="downloadResource('seal_image')"
                >
                  下载当前印章
                </el-link>
              </div>
...
import {
  ...
  downloadCertificateSettingFile,
...
const openEditDialog = (item: CertificateSetting) => {
  ...
  form.background_image_name = item.background_image_name || "";
  form.seal_image_name = item.seal_image_name || "";
...
const downloadResource = async (
  field: "background_image" | "seal_image"
) => {
  if (!editingId.value) return;
  const filename =
    field === "background_image"
      ? form.background_image_name || "证书底图"
      : form.seal_image_name || "电子印章";
  try {
    const blob = (await downloadCertificateSettingFile(
      editingId.value,
      field
    )) as Blob;
    saveBlob(blob, filename);
  } catch {
    ElMessage.error("下载证书资源失败");
  }
};
