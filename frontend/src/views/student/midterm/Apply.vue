...
const downloadMidTermReport = async () => {
  if (!project.value) return;
  try {
    const blob = (await downloadProjectFile(
      project.value.id,
      "mid_term_report"
    )) as Blob;
    saveBlob(blob, project.value.mid_term_report_name || "中期报告");
  } catch {
    ElMessage.error("下载中期报告失败");
  }
};
