...
import { lookupUsers } from "@/api/users/public";
...
const createInitialMember = () => ({ student_id: "", name: "" });
...
export function useProjectApplication() {
  ...
  const confirmedMemberStudentId = ref("");
  ...
  const formData = reactive<FormDataState>(createInitialFormData());
  ...
  // Temp inputs
  const newAdvisor = reactive(createInitialAdvisor());
  const newMember = reactive(createInitialMember());
  ...
  const handleDownloadTemplate = () => {
    ...
    if (currentTemplateUrl.value) {
      window.open(currentTemplateUrl.value, "_blank", "noopener,noreferrer");
    } else {
  ...
  const handleAddNewAdvisor = () => {
    ...
    // Reset
    Object.assign(newAdvisor, createInitialAdvisor());
  };
  ...
  watch(
    ...
    (newVal) => {
      ...
      if (!normalized || normalized !== confirmedMemberStudentId.value) {
        newMember.name = "";
      }
    }
  );
  ...
  const handleSearchNewMember = async () => {
    const studentId = newMember.student_id.trim();
    if (!studentId) {
    ...
    try {
      ...
      if (users.length > 0) {
        const user = users[0];
        newMember.student_id = studentId;
        newMember.name = user.real_name;
        confirmedMemberStudentId.value = studentId;
        ElMessage.success(`已找到学生: ${user.real_name}`);
      } else {
        ElMessage.error("未找到该学号的学生");
        confirmedMemberStudentId.value = "";
        newMember.name = "";
      }
    } catch (error: unknown) {
      console.error("Search failed", error);
  ...
  const handleAddNewMember = () => {
    ...
    if (!newMember.name || studentId !== confirmedMemberStudentId.value) {
      ElMessage.warning("请先查询并确认学生信息");
      return;
    }

    // Check duplicate
    if (formData.members.some((m) => m.student_id === studentId)) {
      ElMessage.warning("该成员已添加");
      return;
    }

    formData.members.push({ student_id: studentId, name: newMember.name });
    // Reset
    Object.assign(newMember, createInitialMember());
    confirmedMemberStudentId.value = "";
  };
  ...
  const handleReset = () => {
    ...
    formData.advisors = [];
    formData.members = [];
    formData.expected_results_data = [];
    formData.attachment_file = null;
    formData.is_key_field = false;
    formData.key_domain_code = "";
    fileList.value = [];
  ...
  const loadData = async (id: number) => {
    ...
    try {
      ...
      if (projectData && (projectData.id || res.code === 200)) {
        ...
        formData.advisors =
          Array.isArray(data.advisors_info) && data.advisors_info.length > 0
            ? data.advisors_info.map((adh, index) => ({
