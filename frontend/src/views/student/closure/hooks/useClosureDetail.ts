...
import {
  downloadProjectAchievementAttachment,
  downloadProjectFile,
  getProjectDetail,
...
export function useClosureDetail() {
  ...
  const handleDownload = async (
    field: "final_report" | "achievement_file"
  ) => {
    if (!projectInfo.id) {
    ...
    const filename =
      field === "final_report"
        ...
        : projectInfo.achievement_file_name || "支撑附件";
    try {
      const blob = (await downloadProjectFile(projectInfo.id, field)) as Blob;
  ...
  const handleAchievementDownload = async (row: AchievementItem) => {
    if (!row.id) return;
    try {
      const blob = (await downloadProjectAchievementAttachment(row.id)) as Blob;
      saveBlob(blob, row.attachment_name || row.title || "成果附件");
    } catch {
      ElMessage.error("下载成果附件失败");
    }
  };
