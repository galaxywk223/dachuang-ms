<template>
  <div
    class="graph-node"
    :class="[nodeTypeClass, { 'is-selected': isSelected, 'is-readonly': isReadonly }]"
  >
    <div class="node-order">{{ orderLabel }}</div>
...
const isSelected = computed(() => Boolean(nodeData.value.selected));
const isReadonly = computed(() => Boolean(nodeData.value.readonly));
const orderLabel = computed(() => nodeData.value.orderLabel || "--");
</script>
...
.graph-node {
  ...
  &.is-selected {
    border-color: rgba(20, 127, 115, 0.72);
    box-shadow: 0 0 0 4px rgba(20, 127, 115, 0.12),
