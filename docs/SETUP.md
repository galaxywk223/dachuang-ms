...
- 数据库：PostgreSQL（默认连接：`localhost:5432`，库名 `dachuang_db`，用户由本地环境变量配置）
...
项目后端数据库连接配置可通过 `backend/.env` 或环境变量覆盖：
`DB_NAME=dachuang_db`、`DB_USER=<local-db-user>`、`DB_PASSWORD=<local-db-password>`、`DB_HOST=localhost`、`DB_PORT=5432`。
...
数据库用户和密码通过 `backend/.env`（推荐）或环境变量设置。

### 2.2 导入本地完整数据导出（可选）

完整数据导出属于本地运行数据，不纳入版本控制。默认路径为 `.local/db/local_data.sql`，可由 `scripts/export_db_full.ps1` 生成，或由本机已有导出文件提供。

Linux/macOS：

```bash
PGPASSWORD='<local-db-password>' psql -h localhost -p 5432 -U <local-db-user> -d dachuang_db -f ./.local/db/local_data.sql
```

Windows PowerShell：

```powershell
$env:PGPASSWORD="<local-db-password>"; psql -h localhost -p 5432 -U <local-db-user> -d dachuang_db -f .\.local\db\local_data.sql
```

说明：

- 导入前需确保数据库 `dachuang_db` 已存在。
- `.local/db/local_data.sql` 由 `pg_dump` 生成，包含 `DROP TABLE IF EXISTS` 等清理语句；导入到目标数据库将覆盖同名对象。
- 完整数据导出文件不存在时，使用 `2.3` 的结构脚本初始化数据库，并按需创建本地演示数据和批次数据。

### 2.3 导入数据库结构（推荐用于干净初始化）

仓库跟踪 `dachuang_db_schema.sql`，仅包含数据库结构，不包含测试数据或用户数据。导入后建议执行一次 `python manage.py migrate` 以确保迁移与代码一致。

Linux/macOS：

```bash
PGPASSWORD='<local-db-password>' psql -h localhost -p 5432 -U <local-db-user> -d dachuang_db -f ./dachuang_db_schema.sql
```

Windows PowerShell：

```powershell
$env:PGPASSWORD="<local-db-password>"; psql -h localhost -p 5432 -U <local-db-user> -d dachuang_db -f .\dachuang_db_schema.sql
```
...
如已导入 `.local/db/local_data.sql`，此步骤可跳过；如需确保迁移与代码一致，可执行一次迁移（通常不会产生变更）。
...
系统登录接口仅允许使用 `employee_id` 登录（不是 Django 默认的 `username`）。数据库中不存在可登录管理员时，可通过 Django Shell 创建本地一级管理员用户：
...
- body 示例：
  - `{"employee_id":"<local-admin-id>","password":"<strong-admin-password>"}`

### 3.4（可选但推荐）设置后端环境变量

后端会读取以下环境变量（不设置也能运行，但不利于多人开发一致性）：

- `DB_NAME` / `DB_USER` / `DB_PASSWORD` / `DB_HOST` / `DB_PORT`：数据库连接信息
- `DJANGO_DEBUG`：调试模式开关，本地默认 `true`，生产环境必须设置为 `false`
- `DJANGO_SECRET_KEY`：JWT 签名 key；生产环境必须设置为强随机值，本地不设置会每次启动随机生成并导致旧 token 失效
- `DJANGO_ALLOWED_HOSTS`：Django 允许访问的域名或 IP，生产环境必须显式配置
- `DJANGO_CORS_ALLOWED_ORIGINS`：允许跨域访问后端的前端来源，生产环境必须显式配置
- `DJANGO_CSRF_TRUSTED_ORIGINS`：可信 CSRF 来源，使用前后端分离域名时需配置
- `DJANGO_CORS_ALLOW_ALL_ORIGINS`：是否允许全部跨域来源；本地默认跟随调试模式，生产环境必须为 `false`
...
- `DJANGO_SECURE_SSL_REDIRECT` / `DJANGO_SESSION_COOKIE_SECURE` / `DJANGO_CSRF_COOKIE_SECURE` / `DJANGO_SECURE_HSTS_SECONDS`：HTTPS 与安全 Cookie 配置；生产环境默认启用 HTTPS 跳转、安全 Cookie 和 HSTS
...
DJANGO_CSRF_TRUSTED_ORIGINS=https://app.example.edu
...
- `DJANGO_DEBUG=false` 时，`DJANGO_SECRET_KEY`、`DJANGO_ALLOWED_HOSTS`、`DJANGO_CORS_ALLOWED_ORIGINS` 必须显式配置。
- `DJANGO_DEBUG=false` 时，`DB_PASSWORD` 必须设置为强密码。
- `DJANGO_DEBUG=false` 时，如配置 `DEFAULT_USER_PASSWORD` 或 `DEFAULT_RESET_PASSWORD`，该值必须使用强密码，且长度不得少于 12 位。
- 生产环境默认启用 HTTPS 跳转、安全 Cookie 和 HSTS；反向代理需传递 `X-Forwarded-Proto=https`。
...
如 `.local/db/local_data.sql` 已包含批次数据，可跳过本节。

手动创建批次需使用本地一级管理员登录后调用批次接口。

Linux/macOS/WSL（curl 示例）：

```bash
# 1) 登录；响应中的 token 位于 data.access_token
curl -s -X POST http://localhost:8000/api/v1/auth/login/ \
  -H 'Content-Type: application/json' \
  -d '{"employee_id":"<local-admin-id>","password":"<strong-admin-password>"}'
```
...
```powershell
$login = Invoke-RestMethod -Method Post -Uri "http://localhost:8000/api/v1/auth/login/" -ContentType "application/json" -Body '{"employee_id":"<local-admin-id>","password":"<strong-admin-password>"}'
...
3. 用户和密码是否匹配本地 `backend/.env` 配置
4. 数据库主机、端口和连接用户是否与环境变量一致
...
- Windows：按 `3.5` 使用 `python manage.py runserver` 启动

### 6.4 登录失败（学号/工号或密码错误）

登录接口只接受 `employee_id` + `password`：

- 确认用户的 `employee_id` 存在
- 确认密码正确
- 确认请求体未沿用旧版 `role` 参数示例
...
- 本地管理员用户：重新用 `python manage.py shell` 创建
