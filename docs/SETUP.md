# Dachuang-MS 本地配置与运行指南（从 GitHub 获取源码开始）

> 目标：仅通过 GitHub 获取源码（下载 ZIP 或克隆仓库），完成本地运行（后端 + 前端 + 数据库）。
...
在 GitHub 仓库页面（`https://github.com/galaxywk223/learning-analytics-system`）选择 `Code` → `Download ZIP`，下载并解压后进入项目根目录（应包含 `backend/`、`frontend/`）。

命令执行位置约定：

- 下文命令默认在“项目根目录”执行（即与 `backend/`、`frontend/` 同级的目录）。
- 若解压后的目录名与示例不一致，将 `cd dachuang-ms` 替换为实际目录名即可。

### 2.2 克隆仓库

```bash
git clone https://github.com/galaxywk223/learning-analytics-system.git dachuang-ms
cd dachuang-ms
```

说明：

- `backend/venv/`、`frontend/node_modules/`、`.local/` 都在 `.gitignore` 中，**GitHub 不会包含这些目录**，需要按下文重新生成。

## 3. 初始化数据库（PostgreSQL）

项目后端默认连接配置写在 `backend/config/settings.py`：  
`NAME=dachuang_db`、`USER=postgres`、`PASSWORD=123456`、`HOST=localhost`、`PORT=5432`。
...
如不使用 `postgres/123456`，需同步修改 `backend/config/settings.py` 的数据库连接信息。
...
Linux/macOS/WSL（推荐）：使用脚本启动（自动创建 venv、安装依赖，并以 `0.0.0.0:8000` 启动服务）。
...
### 7.4 登录失败（学号/工号或密码错误 / role_mismatch）
...
### 7.5 pip 安装依赖失败（lxml / pillow 等编译问题）
