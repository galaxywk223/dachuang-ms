...
$outputFile = if ($env:OUTPUT_FILE) { $env:OUTPUT_FILE } else { (Join-Path $repoRoot ".local\db\local_data.sql") }
...
if (-not $dbPassword) {
  $msg = "DB_PASSWORD must be set before exporting the full database."
  if ($DryRun) {
    Write-Host $msg
    exit 0
  }
