...
def _as_decimal(value, default=Decimal("0")):
    ...
    try:
        ...
        raise ValueError("经费金额格式错误") from exc


def _project_total_budget(project):
    budget = (
        project.approved_budget
        if project.approved_budget is not None
        else project.budget
    )
    return _as_decimal(budget)
...
class BudgetService:
    ...
    def submit_expenditure(project, user, data):
        ...
        new_amount = _as_decimal(data.get("amount", 0))
        if new_amount <= 0:
            raise ValueError("经费金额必须大于0")
        total_budget = _project_total_budget(project)
        if total_expenditure + new_amount > total_budget:
            raise ValueError(
                f"支出总额不能超过预算额度（预算：{total_budget}，"
