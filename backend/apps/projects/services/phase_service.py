...
class ProjectPhaseService:
    ...
    @staticmethod
    def get_in_progress(project: Project, phase: str) -> Optional[ProjectPhaseInstance]:
        return (
    ...
    def ensure_current(
        ...
        *,
        ...
        current = ProjectPhaseService.get_in_progress(project, phase)
        if current:
            update_fields = []
        ...
        latest = ProjectPhaseService.get_current(project, phase)
        next_attempt = 1 if not latest else int(latest.attempt_no) + 1
