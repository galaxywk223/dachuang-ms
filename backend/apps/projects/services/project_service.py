...
from apps.projects.upload_validation import validate_project_document_file
...
class ProjectService:
    ...
    def apply_closure(project, final_report, is_draft=False):
        ...
        elif not is_draft and not project.final_report:
    ...
    def apply_mid_term(project, mid_term_report, is_draft=False):
        ...
        if mid_term_report is not None:
            project.mid_term_report = validate_project_document_file(
                mid_term_report,
                label="中期报告",
                max_size_mb=5,
                error_class=ValueError,
            )
        elif not is_draft and not project.mid_term_report:
    ...
    def get_budget_stats(project):
        ...
        expenditures = project.expenditures.filter(
            is_deleted=False,
            status__in=[
        ...
        budget = (
            ...
            if project.approved_budget is not None
            else project.budget
        )
        if budget is None:
            total_budget = Decimal("0")
        else:
            total_budget = budget if isinstance(budget, Decimal) else Decimal(str(budget))
        used_amount = expenditures.aggregate(total=models.Sum("amount"))[
            "total"
        ] or Decimal("0")
