...
class ProjectValidationService:
    ...
    def validate_project_application(project, user, is_update=False):
        ...
        limit_rules = (
            settings_service.get_setting("LIMIT_RULES", batch=project.batch) or {}
        )
        validation_rules = (
            settings_service.get_setting("VALIDATION_RULES", batch=project.batch)
            or {}
        )
    ...
    def _validate_title_duplication(project, is_update):
        ...
        # Submitted creates validate after serializer.save(), so exclude the current
        # row whenever it already exists to avoid treating itself as a duplicate.
        if project.id:
    ...
    def _validate_title_format(title, validation_rules):
        ...
        """
        # 标题长度校验
        min_length = non_negative_int(validation_rules.get("title_min_length"), 0)
        max_length = non_negative_int(validation_rules.get("title_max_length"), 200)
    ...
    def _validate_advisors(project, limit_rules, is_update):
        ...
        # 指导教师数量限制
        max_advisors = non_negative_int(limit_rules.get("max_advisors"), 2)
        if len(advisors) > max_advisors:
            errors.append(f"指导教师数量不能超过{max_advisors}人")

        # 指导教师在研项目数限制
        max_teacher_active = non_negative_int(limit_rules.get("max_teacher_active"), 5)
        ...
        for advisor in advisors:
            ...
            active_count = Project.objects.filter(
                advisors__user_id=advisor_user_id,
                is_deleted=False,
                status__in=[
            ...
            if max_teacher_active and active_count >= max_teacher_active:
                errors.append(
    ...
    def _validate_members(project, limit_rules, is_update):
        ...
        # 成员数量限制
        max_members = non_negative_int(limit_rules.get("max_members"), 5)
        if len(members) > max_members:
            errors.append(f"项目成员数量不能超过{max_members}人（不含负责人）")

        # 学生参与项目数限制
        max_student_member = non_negative_int(limit_rules.get("max_student_member"), 1)
        ...
        for member_id in all_members:
            ...
            if max_student_member and total_count >= max_student_member:
                try:
