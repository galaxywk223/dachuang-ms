...
def parse_optional_budget(value, message):
    if value in (None, ""):
        return None
    try:
...
def parse_optional_node_id(value, message):
    if value in (None, ""):
        return None
    try:
        node_id = int(value)
...
class ProjectWorkflowMixin:
    ...
    def expert_summary(self, request, pk=None):
        ...
        self._assert_assigned_admin(project, review_type, request.user)

        node_id_param = request.query_params.get("node_id")
        try:
            node_id = parse_optional_node_id(node_id_param, "node_id格式错误")
        except ValueError as exc:
            return Response(
    ...
    def workflow_publish_establishment(self, request, pk=None):
        ...
        try:
            approved_budget_val = parse_optional_budget(
                approved_budget,
                "approved_budget格式错误",
            )
        except ValueError as exc:
    ...
    def workflow_finalize_midterm(self, request, pk=None):
        ...
        try:
            target_node_id = parse_optional_node_id(
        ...
        try:
            if action_type == "approve":
                ...
                ReviewService.reject_review(
                    ...
                    reason,
                    target_node_id=target_node_id,
                    allow_any_previous_target=True,
                )
        except ValueError as exc:
            return Response(
                {"code": 400, "message": str(exc)},
