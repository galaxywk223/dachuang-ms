...
class AchievementManagementViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if not user.is_admin:
            return ProjectAchievement.objects.none()
        if user.is_admin and not _has_school_admin_scope(user):
        ...
        if year:
            ...
            if parsed_year is None:
                return ProjectAchievement.objects.none()
            queryset = queryset.filter(project__created_at__year=parsed_year)
    ...
    def list(self, request, *args, **kwargs):
        ...
        if request.user.is_admin and not _has_school_admin_scope(request.user):
            projects_qs = projects_qs.filter(leader__college=request.user.college)
        ...
        if year:
            ...
            if parsed_year is None:
                ...
                projects_qs = projects_qs.filter(created_at__year=parsed_year)
            ...
            data.append(
                {
                    ...
                    "leader": ach.project.leader.real_name
                    if ach.project.leader
                    else "",
                    "college": ach.project.leader.college if ach.project.leader else "",
                    "type": ach.achievement_type.label
                    if ach.achievement_type
                    else "",
                    "title": ach.title,
