...
def _parse_approved_budget(value):
    if value in (None, ""):
        raise ValueError("请填写批准经费")
    try:
        amount = Decimal(str(value))
    except (InvalidOperation, TypeError, ValueError) as exc:
        raise ValueError("批准经费格式不正确") from exc
    if not amount.is_finite():
        raise ValueError("批准经费格式不正确")
    if amount < 0:
        raise ValueError("批准经费不能为负数")
    return amount
...
class ReviewViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        return Response(
            {"code": 405, "message": "请通过项目流程操作创建审核记录"},
            status=status.HTTP_405_METHOD_NOT_ALLOWED,
        )

    def update(self, request, *args, **kwargs):
        return Response(
            {"code": 405, "message": "请通过审核操作更新审核记录"},
            status=status.HTTP_405_METHOD_NOT_ALLOWED,
        )
    ...
    def get_queryset(self):
        ...
        queryset = (
            ...
            .select_related(
                ...
                "reviewer",
                "workflow_node",
                "workflow_node__role_fk",
                "phase_instance",
            )
            .prefetch_related("project__advisors__user")
        )
    ...
    def review(self, request, pk=None):
        ...
        min_len = non_negative_int(
            review_rules.get("teacher_application_comment_min", 0)
        )
        review_role_code = (
            review.workflow_node.get_role_code() if review.workflow_node else None
        )
        review_role_scope = (
            review.workflow_node.role_fk.scope_dimension
            if review.workflow_node and review.workflow_node.role_fk
            else None
        )
        if (
            min_len
        ...
        try:
            ...
            if (
                ...
                and review.review_type == Review.ReviewType.APPLICATION
                and (
                    review_role_code == "LEVEL1_ADMIN"
                    or review_role_scope == "SCHOOL"
                )
                and not review.is_expert_review
            ):
                try:
                    approved_budget_val = _parse_approved_budget(approved_budget)
                except ValueError as exc:
                    return Response(
                        {"code": 400, "message": str(exc)},
                        status=status.HTTP_400_BAD_REQUEST,
            ...
            if action_type == "approve":
                ...
                if result and approved_budget_val is not None:
    ...
    def revise(self, request, pk=None):
        ...
        if review.status != desired_status:
            return Response(
                {"code": 400, "message": "审核结果修订不能切换通过/驳回状态"},
                status=status.HTTP_400_BAD_REQUEST,
            )
    ...
    def batch_review(self, request):
        ...
        review_ids = positive_int_list(review_ids)
        if not review_ids:
            return Response(
                {"code": 400, "message": "review_ids不合法"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        if action_type == "reject":
            ...
            if target_node_id not in (None, ""):
                target_node_id = optional_positive_int(target_node_id)
                if target_node_id is None:
                    return Response(
                        {"code": 400, "message": "target_node_id不合法"},
                        status=status.HTTP_400_BAD_REQUEST,
                    )
            else:
                target_node_id = None
        ...
        success = 0
        failed = []
        for review in self.get_queryset().filter(
            ...
            try:
                ...
                if action_type == "approve":
                    ...
                    result = ReviewService.reject_review(
                        review, request.user, comments, target_node_id
                    )
            except ValueError as exc:
                failed.append({"id": review.id, "reason": str(exc)})
                continue
            if not result:
                failed.append({"id": review.id, "reason": "审核流程处理失败"})
                continue
            NotificationService.notify_review_result(
                review.project, action_type == "approve", comments
            )
    ...
    def submit_to_level1(self, request):
        ...
        review = (
            ...
            .filter(project_id=parsed_project_id)
            .first()
        )
        if not review:
            return Response(
