...
class ReviewService:
    ...
    def _update_project_status_for_node(project, node, phase):
        ...
        role_code = getattr(node, "role", None)
        if not role_code and hasattr(node, "get_role_code"):
            role_code = node.get_role_code()
        role_scope = ReviewService._role_scope_for_node(node)

        if phase == ProjectPhaseInstance.Phase.APPLICATION:
            if role_code == "TEACHER":
                project.status = Project.ProjectStatus.TEACHER_AUDITING
            elif role_code == "LEVEL2_ADMIN" or role_scope == "COLLEGE":
                project.status = Project.ProjectStatus.COLLEGE_AUDITING
            elif role_code == "LEVEL1_ADMIN" or role_scope == "SCHOOL":
    ...
    @staticmethod
    def _ensure_review_matches_current_node(review, phase_instance):
        if phase_instance.state != ProjectPhaseInstance.State.IN_PROGRESS:
            raise ValueError("阶段流程已结束，无法处理该审核记录")

        if review.workflow_node_id:
            if not phase_instance.current_node_id:
                raise ValueError("流程状态异常：缺少当前节点")
            if review.workflow_node_id != phase_instance.current_node_id:
                raise ValueError("审核记录不属于当前流程节点")
            return review.workflow_node_id

        if not phase_instance.current_node_id:
            inferred_node = WorkflowService.get_node_by_code(
                phase_instance.phase, phase_instance.step, review.project.batch
            )
            if inferred_node:
        ...
        if not phase_instance.current_node_id:
            raise ValueError("流程状态异常：缺少当前节点")
        return phase_instance.current_node_id

    @staticmethod
    def _ensure_pending_review(review):
        if review.status != Review.ReviewStatus.PENDING:
            raise ValueError("审核记录已处理，无法重复审核")

    @staticmethod
    def _validate_assignment_node(project, phase, node_obj):
        if not node_obj:
            return
        workflow = node_obj.workflow
        if workflow.phase != phase or workflow.batch_id != project.batch_id:
            raise ValueError("目标节点不属于当前项目流程")

    @staticmethod
    def _validate_reject_target(
        review, phase_instance, target_node_id, *, allow_any_previous=False
    ):
        if target_node_id is None:
            return
        if not phase_instance.current_node_id:
            raise ValueError("流程状态异常：缺少当前节点")
        nodes = WorkflowService.get_nodes(phase_instance.phase, review.project.batch)
        node_order = {node.id: idx for idx, node in enumerate(nodes)}
        current_index = node_order.get(phase_instance.current_node_id)
        target_index = node_order.get(target_node_id)
        if (
            current_index is None
            or target_index is None
            or target_index >= current_index
        ):
            raise ValueError("退回目标不属于当前节点可退回范围")
        if allow_any_previous:
            return
        allowed_targets = WorkflowService.get_reject_target_nodes(
            phase_instance.current_node_id,
            phase_instance.phase,
            review.project.batch,
        )
        if target_node_id not in {target.id for target in allowed_targets}:
            raise ValueError("退回目标不属于当前节点可退回范围")
    ...
    def approve_review(
        ...
        审核通过 - 使用动态工作流引擎
        """
        ReviewService._ensure_pending_review(review)
        ...
        current_node_id = ReviewService._ensure_review_matches_current_node(
    ...
    def reject_review(
        ...
        target_node_id=None,
        *,
        allow_any_previous_target=False,
    ):
        """
        审核不通过 - 使用动态工作流引擎
        ...
        ReviewService._validate_reject_target(
            review,
            phase_instance,
            target_node_id,
            allow_any_previous=allow_any_previous_target,
        )
    ...
    def _build_admin_review_filter(admin_user):
        ...
        # 获取管理员角色
        role = admin_user.role_fk
        role_code = admin_user.get_role_code()
        if not role or not role_code:
            return None
        ...
        for node in nodes:
            ...
            if scope_dimension == "COLLEGE":
                scope_value = admin_user.managed_scope_value
                scope_college = (
                    scope_value.value
                    if scope_value and scope_value.dict_type.code == "college"
                    else admin_user.college
                )
                if not scope_college:
                    return None
                cond &= Q(project__leader__college=scope_college)
            elif scope_dimension not in (None, "", "SCHOOL"):
                return None
    ...
    def assign_project_to_group(
        ...
        with transaction.atomic():
            for pid in project_ids:
                ...
                ReviewService._validate_assignment_node(project, phase, node_obj)
                if (
                    target_node_id
                    and phase_instance.current_node_id
                    and target_node_id != phase_instance.current_node_id
                ):
                    raise ValueError("目标节点不属于当前流程节点")
                if not node_obj and phase_instance.step:
                    ...
                    if inferred_node:
                        ...
                        ReviewService._validate_assignment_node(
