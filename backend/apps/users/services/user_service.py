...
ALLOWED_USER_IMPORT_EXTENSIONS = {".xls", ".xlsx", ".xlsm", ".xltx", ".xltm"}
MAX_USER_IMPORT_SIZE = 10 * 1024 * 1024


def _normalize_import_filename(file):
    raw_name = str(getattr(file, "name", "") or "")
...
def _validate_import_file(file):
    filename = _normalize_import_filename(file)
    suffix = Path(filename).suffix.lower()
    if suffix not in ALLOWED_USER_IMPORT_EXTENSIONS:
        raise ValueError("仅支持 xls/xlsx/xlsm/xltx/xltm 文件")

    file_size = getattr(file, "size", 0) or 0
    if file_size > MAX_USER_IMPORT_SIZE:
...
class UserService:
    ...
    def reset_password(self, user, new_password=None):
        ...
        if password_errors:
            raise ValueError("密码不符合强度要求：" + "；".join(password_errors))
    ...
    def import_users(
        ...
        ext = _validate_import_file(file)
        data = file.read()
        if not data:
        ...
        if not role_obj:
            raise ValueError("导入角色不存在")
        if role_obj.code == User.UserRole.EXPERT:
            raise ValueError("不支持直接导入专家，请先导入教师")
        if not settings.DEFAULT_USER_PASSWORD:
            raise ValueError("批量导入用户前必须配置 DEFAULT_USER_PASSWORD")
        password_errors = get_password_validation_errors(settings.DEFAULT_USER_PASSWORD)
        if password_errors:
            raise ValueError("默认用户密码不符合强度要求：" + "；".join(password_errors))
        ...
        with transaction.atomic():
            for row_idx, row in enumerate(rows[1:], start=2):
                ...
                try:
                    ...
                    logger.exception("Failed to import user row %s", row_idx)
                    errors.append(f"Row {row_idx}: 导入失败，请检查该行数据")
