...
def migrate_user_roles(apps, schema_editor):
    ...
    for user in User.objects.all():
        # old_role 字段在数据库操作时已经保存了原来的 role 值
        if user.old_role and user.old_role in roles_dict:
            user.role = roles_dict[user.old_role]
            user.save()
...
def reverse_migrate_user_roles(apps, schema_editor):
    ...
    for user in User.objects.all():
        if user.old_role:
            # 这里需要注意，反向迁移时直接赋值字符串会失败
            # 因为字段类型已经改变，这个函数主要是为了文档说明
            pass
    ...
    operations = [
        ...
        migrations.AddField(
            ...
            field=models.CharField(
                blank=True, max_length=20, verbose_name="旧角色代码（迁移用）"
            ),
        ),
        migrations.CreateModel(
            name="Role",
