...
CERTIFICATE_DOWNLOAD_FIELDS = {"background_image", "seal_image"}
...
class SystemSettingViewSet(viewsets.ModelViewSet):
    ...
    def _resolve_requested_batch(self, batch_id):
        if not batch_id:
            return None, None
        ...
        batch = ProjectBatch.objects.filter(id=parsed_batch_id).first()
        if not batch:
            return None, Response(
    ...
    def create(self, request, *args, **kwargs):
        return Response(
            {"code": 405, "message": "请通过按编码配置接口维护系统配置"},
    ...
    def effective(self, request):
        ...
        data = {}
        batch_id = request.query_params.get("batch_id")
        batch, denied = self._resolve_requested_batch(batch_id)
...
class ProjectBatchViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        can_include_deleted = include_deleted in ["1", "true", "yes"]
        if can_include_deleted and not has_school_admin_scope(self.request.user):
            can_include_deleted = False
        if not can_include_deleted:
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        if instance.status == ProjectBatch.STATUS_ARCHIVED:
            if Project.objects.filter(batch_id=instance.id).exists():
                ...
                instance.is_active = False
                instance.save(
                    update_fields=["is_deleted", "is_current", "is_active"]
                )
                return Response({"code": 200, "message": "批次已归档删除"})
            instance.delete()
            return Response({"code": 200, "message": "批次已删除"})
        return Response(
...
class CertificateSettingViewSet(viewsets.ModelViewSet):
    ...
    def download_file(self, request, pk=None, field_name=None):
        ...
        if field_name not in CERTIFICATE_DOWNLOAD_FIELDS:
            return Response(
                {"code": 404, "message": "证书资源类型不存在"},
                status=status.HTTP_404_NOT_FOUND,
            )
        file_field = getattr(setting, field_name, None)
        if not file_field:
            return Response(
                {"code": 404, "message": "证书资源不存在"},
                status=status.HTTP_404_NOT_FOUND,
            )
        return file_field_download_response(
            file_field,
            missing_message="证书资源不存在",
        )
