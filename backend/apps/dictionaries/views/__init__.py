...
class DictionaryTypeViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        return Response(
            {"detail": "字典类型不支持接口直接创建"},
            status=status.HTTP_405_METHOD_NOT_ALLOWED,
        )

    def update(self, request, *args, **kwargs):
        return Response(
            {"detail": "字典类型不支持接口直接修改"},
            status=status.HTTP_405_METHOD_NOT_ALLOWED,
        )
...
class DictionaryItemViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        dict_type_id = self.request.query_params.get("dict_type")
        if dict_type_id:
            parsed_type_id = optional_positive_int(dict_type_id)
            if parsed_type_id is None:
                return queryset.none()
            queryset = queryset.filter(dict_type_id=parsed_type_id)
        return queryset

    def destroy(self, request, *args, **kwargs):
        instance = self.get_object()
        if instance.dict_type.is_system:
            return Response(
                {"detail": "系统内置字典条目不支持直接删除，可改为停用"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        try:
    ...
    def bulk_create(self, request):
        ...
        if dict_type_id:
            parsed_type_id = optional_positive_int(dict_type_id)
            dict_type = (
                DictionaryType.objects.filter(id=parsed_type_id).first()
                if parsed_type_id is not None
                else None
            )
        else:
